# Drupal AI Demo: Compliance Checks

A [Drupal CMS](https://www.drupal.org/project/cms) 2.x demo on [Lagoon](https://lagoon.sh) showing AI-assisted content simplification with [amazee.ai](https://amazee.ai): content is reviewed against compliance rules — violations flagged and fixes suggested by [AI Automators](https://www.drupal.org/project/ai).

## How it's built

The whole site is defined by the bundled recipe at `recipes/drupal-org-ai-compliance/` — Drupal CMS starter plus the AI modules, an `article` content type with compliance fields and AI automators, and demo content as YAML. `drush site:install <recipe>` produces the complete demo from a clean database.
...
docker compose exec cli bash -c 'drush -n si /app/recipes/drupal-org-ai-compliance && drush -y cr'
```
